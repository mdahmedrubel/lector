 ...
 */
function Lector_breadcrumbs($class = '') {
    // Settings
    $separator	 = '&gt;';
    $breadcrums_id	 = 'breadcrumbs';
    $breadcrums_class	 = 'breadcrumb bg-transparent mb-0 d-flex flex-wrap justify-content-center align-items-center' . $class;
    $home_title	 = esc_html__('Home', 'lector');
    ...
    if ( !is_front_page() ) {
        ...
        // Home page
        echo '<li><a class="bread-link bread-home" href="' . esc_url(get_home_url()) . '" title="' . $home_title . '">' . $home_title . '</a></li>';
        ...
        if ( is_author() ) {
            ...
            echo '<li class="active-breadcrumb item-archive"><span class="bread-current bread-archive">' . $custom_tax_name . '</span></li>';
            ...
            // Paginated archives
            echo '<li class="item-current item-current-' . get_query_var('paged') . '"><span class="bread-current bread-current-' . get_query_var('paged') . '" title="Page ' . get_query_var('paged') . '">'.esc_html__('Page','lector') . ' ' . get_query_var('paged') . '</span></li>';

        } elseif ( is_404() ) {
            // 404 page
            echo '<li class="mr_left">' . 'Error 404' . '</li>';
        }elseif ( is_search() ) {
           // search page
           echo '<li class="mr_left">' . 'Search Result' . '</li>'; 
        }
        echo '</ul>';
    }
}